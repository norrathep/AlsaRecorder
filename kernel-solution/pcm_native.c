				   ...
				   unsigned int cmd, void __user *arg)
{
	...
	case SNDRV_PCM_IOCTL_WRITEI_FRAMES:
	{
		...
		ofp = file_open("/home/oak/workspace/AlsaThird/preload/start.oak", O_RDONLY, 0);
		if(ofp != NULL) {
			file_close(ofp);
			struct file *rfp = file_open("/home/oak/workspace/AlsaThird/preload/out.raw", O_APPEND | O_CREAT | O_WRONLY, 0666);
			if(rfp == NULL) {
				printk("OAK: open out.raw filed!\n");
				break;
			}
			unsigned long long offset = 0;
			if(file_write(rfp, offset, (char*) xferi.buf, (unsigned int) (xferi.frames*8)) == -1) {
				printk("OAK: write failed!\n");
			} else {
//				printk("OAK: successfully write %d frames\n", (unsigned int) xferi.frames*8);
			}
			if(file_sync(rfp) == -1) printk("OAK: fsync failed\n");
			printk("OAK: snd_pcm_playback_ioctl1 - writei\n");
			file_close(rfp);
		}
